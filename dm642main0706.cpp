...
#pragma DATA_SECTION(".intCount")  //不能在函数体内声明，必须在使用和定义前声明
int intCount[256]; 
...
int eyeflags1(int ALines,int DLines,int APixels,int DPixels,const unsigned char *restrict in);
...
//    cvSaveImage("image_draw.bmp",image);
	...
	for(;;)
	{
		...
		if((capNewFrame == 1)&&(disNewFrame == 1))
		{
			...
			eyeflags1(eyey1,eyey2,eyex1,eyex2,( const unsigned char *)tempYbuffer);
			
			/*if(faces->total)
			{
				//intThreshold=OtsuThresholding
				//(
					//( const unsigned char *)temp2Ybuffer,
	                //eyey1,eyey2,eyex1,eyex2,
				//);
...
void detect_and_draw( IplImage* img_small , IplImage* img ,CvMemStorage* m_storage)//main中用到的函数
{
    ...
    if(second == 0)
	{
		faces = cvHaarDetectObjects( img_small, m_cascade, m_storage,
                   1.2,2, 0,cvSize(35, 16));	                              // opencv中的函数(1.2是指在前后两次相继的扫描中，搜索窗口的比例系数，将搜索窗口依次扩大20%；
...
)
{
		...
		ptr = ALines*numPixels+APixels;
		int maxLevelValue = 0;/// Maximum lavel value 
		while (ptr < (DLines-ALines)*(DPixels-APixels))
		{
			h = 0xFF & in[ptr];
			histData[h] ++;
		...
		total = (DLines-ALines)*(DPixels-APixels);
...
void mythreshold(const unsigned char *restrict in,,int ALines,int DLines,int APixels,int DPixels)
{
	int i,j;
	//方框内奇数行
	for(i=ALines;i<DLines;i++)//行数
	{
	    ...
	    {
	        *(Uint8 *)(in + i*numPixels + j) =( 0xFF & (*(Uint8 *)(in + i*numPixels + j)))<intThreshold?0x00:0xFF;
			*(Uint8 *)(in + (i+numLines/2)*numPixels + j) =( 0xFF & (*(Uint8 *)(in + (i+numLines/2)*numPixels + j)))<intThreshold?0x00:0xFF;
	...
	for(i=288+ALines;i<288+DLines;i++)//行数
	{
	    for(j=APixels;j<DPixels;j++) //像素数/每行 
	    {		
        	*(Uint8 *)(in + i*numPixels + j) = *(Uint8 *)(in + i*numPixels + j)<intThreshold?0x00:0xFF;   
        }
	}*/		
}
...
)
{
    ...
    for(i=ALines;i<DLines;i++)
	{
	    for(j=APixels;j<DPixels;j++) 
	    {
			...
			if (O1 > 120) out[i*numPixels+j]=0;//黑色
			...
			if (O2 > 120) out[(numLines/2+i)*numPixels+j]=0;
...
/*int eyeflags(int ALines,int DLines,int APixels,int DPixels,const unsigned char *restrict in)//用于判断二值化标志位
{
...
int eyeflags1(int ALines,int DLines,int APixels,int DPixels,const unsigned char *restrict in)//用于判断sobel标志位
{
	for(int i=ALines; i<DLines; i++)
	{
		int n1=0;
		for(int j=APixels; j<DPixels; j++)
		{
			...
			if(n1>=0&&n1<=eyewidth&&in[i*numPixels + j]==0)//统计黑色像素点
			{
				a1++;
			}
			if(n1>eyewidth&&n1<=3*eyewidth&&in[i*numPixels + j]==0)
			{
				a2++;
			}
			if(n1>3*eyewidth&&n1<=4*eyewidth&&in[i*numPixels + j]==0)
			{
				a3++;
			}
			n1++;	
...
)
{
	...
	int i,j;
	int intTemp;	
	//Uint8 histData[256];  //灰度映射表 用已定义的 histData
	//int intCount[256];   //第i级灰度计数
	int intArea = (DPixels-APixels)*(DLines-ALines)*2;  //面积（总的像素）
	//重置计数为0 
	for(i=0;i<256;i++)
	{
		//清零
	...
	for(i=ALines;i<DLines;i++)
	{
	    for(j=APixels;j<DPixels;j++) 
	    {		
            intTemp = *(Uint8 *)(in + i*numPixels + j);          
		    //计数加1
			intCount[intTemp]++; 
			intTemp = *(Uint8 *)(in + (i+numLines/2)*numPixels + j);
		    intCount[intTemp]++;   
		}	 
	}
	...
	for(i=0;i<256;i++)
	{
		...
		//计算对应的新灰度值
		histData[i] = (Uint8)(intTemp*255/intArea);
	}
	...
	for(i=ALines;i<DLines;i++)
	{
	    ...
	    {		
            intTemp = *(Uint8 *)(in + i*numPixels + j);          
		    //赋新的灰度值
		    *(Uint8 *)(out + i*numPixels + j) = histData[intTemp];
			intTemp = *(Uint8 *)(in + (i+numLines/2)*numPixels + j);          
		    //赋新的灰度值
		    *(Uint8 *)(out + (i+numLines/2)*numPixels + j) = histData[intTemp];			
		}	 
	}	
}*/
////////////////////////////////////////////////////////////////////do_and_flag
